...
Open `sample-output/resume.html` in a browser to preview it. From the repository root, export it with the Playwright-based converter:

```bash
uv sync
uv run playwright install chromium
uv run python html_to_pdf.py \
  examples/sample-output/resume.html \
  examples/sample-output/resume.pdf
```
